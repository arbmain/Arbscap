...
async def reset_pin(
    ...
    try:
        # 🔹 Normalize email
        normalized_email = pin_data.email.strip().lower()
        ...
        # Fetch OTP (ignore `used` because it was already verified)
        otp_record = db.query(OTP).filter(
            func.lower(OTP.email) == normalized_email,
            OTP.otp_code == pin_data.otp_code,
...
def create_transfer(
    ...
    if transfer_data.to_email:
        recipient = db.query(User).filter(func.lower(User.email) == transfer_data.to_email.strip().lower()).first()
    else:
